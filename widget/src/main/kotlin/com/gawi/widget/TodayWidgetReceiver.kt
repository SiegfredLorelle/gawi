 ...
 * a count of the app's exported components: such a count is wrong anyway, since
 * `PreviewActivity` and `ProfileInstallReceiver` are both exported and Glance
 * merges an exported `GlanceRemoteViewsService` besides. See docs/ux/widget.md
 * §5: it adds no permission, carries no data in its intents, and every action it
 * can receive is an `AppWidgetManager` broadcast.
 */
