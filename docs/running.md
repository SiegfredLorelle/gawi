...
- **`make` needs a POSIX shell.** The Makefile uses `[ -n "$CI" ]`, `grep` and
  ...
  | `make lint` | `bash scripts/check-history.sh`, `bash scripts/check-citations.sh`, `bash scripts/check-tests.sh` and `bash scripts/check-docs.sh` then `gradlew.bat spotlessCheck detekt lint :app:assembleDebug` |
