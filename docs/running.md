...
- **`make` needs a POSIX shell.** The Makefile uses `[ -n "$CI" ]`, `grep` and
  ...
  | `make lint` | `bash scripts/check-history.sh`, `bash scripts/check-citations.sh` and `bash scripts/check-tests.sh` then `gradlew.bat spotlessCheck detekt lint :app:assembleDebug` |
