...
| Reminder | the notification's small icon is alpha-only, so a silhouette — of the launcher mark, which is what holds at 24 dp | still | **built** as the gill cluster, no cut-outs — `app/src/main/res/drawable/ic_reminder.xml` |
| Launcher | visual-identity §7.1's mark — one gill cluster, no face — on the light scheme's `onPrimaryContainer` (`#00353A`), the same geometry serving the monochrome layer | still | **built**, the face retired and the ground inverted — `app/src/main/res/mipmap-anydpi/ic_launcher.xml` |
...
will restore. Momo is §2's own colours in both, and the only thing that varies
them is the regenerating desaturation, done arithmetically in `drawMomo` so a
test can measure it.
