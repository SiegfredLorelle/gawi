...
- Missing a day/week resets the streak. Grace mechanics: **gills** (§8, OQ-3), scheduled for 1.0.0 (§5, step 3).
...
today, which is in 1.x rather than here. **The launcher is the one item still
open** — the mark is chosen, a single gill cluster, but its ink and its ground
are being decided by looking, because removing the face removed the contrast
that let a pale mark sit on a pale ground.
...
- **OQ-3, decided: gills.** *Streak freeze / grace day mechanics.* A gill is a spare life. A daily habit grows one per seven clean days, capped at three; a missed day burns one automatically and the streak count is preserved; with none left the streak breaks as today and Momo regenerates. Weekly habits get the same in weeks. Nothing to tap: Momo's gills show the count, which is the visual the character already has. Scheduled in §5's 1.0.0 step 3, canvas page first. **Two numbers rode on this question, not one**, and only the first is now answered. `Mascot.REGENERATING_WINDOW_DAYS = 3` is a separate guess at how long a broken streak keeps Momo regenerating, and nothing on screen has ever distinguished a user recovering from a broken streak from one merely pottering, because Phase 0 draws three faces and folds `regenerating` onto `neutral` ([docs/ux/today-view.md](ux/today-view.md) §4) — the window is decided, tested and unobservable. It is re-examined against the gills in step 3, and its KDoc names that decision.
...
- **Streak resets causing abandonment.** Mitigation: OQ-3's escape hatch — gills, decided 2026-09-03 and scheduled for 1.0.0 (§5, §8); until it is built the risk stands as it did. **Half of this mitigation is gone**: "observe own behavior in a trial" had no replacement and was not given one, so what remains is the escape hatch without the evidence that would have sized it.
