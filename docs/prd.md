...
- Missing a day/week resets the streak once the run has no spare life left to spend. Grace mechanics: **gills** (§8, OQ-3); the count and the drawing are both built.
...
variable font over the ten roles §5 of
[visual-identity.md](ux/visual-identity.md) names; the experiment that gated the
typeface — can a Glance widget be handed a bundled font at all — ran and the
answer was no, because a Glance widget silently drops one, so the widget draws
its names as bitmaps set in the same face (visual-identity.md §2, §5). The
widget set §7.4 scoped is built and derives its palette from the app's
([docs/ux/widget.md](ux/widget.md) §6–§7). The split is what bounded the wait on
a design decision rather than removing it, which is the cost worth stating.
...
- Streak milestone celebrations (7, 30, 100 days; 4, 12, 52 weeks). **Built 2026-08-29**, designed first on the canvas's "Milestone celebration" page: the tank plays a longer, larger sequence than the day-complete one (built 2026-08-26 — the tank marks the mood entering thriving, once per screen session), the copy line names the milestone for the run, and the row's streak badge pulses; Momo's habitat has life in it that keeps the mood's tempo. All per `docs/ux/momo.md` §4, §5 and §6.
...
R8 needs two keep rules here, and both took a device to find. The first: R8
...
second is owed — R8 merges the three `GlanceAppWidget` subclasses into one
class, so a push draws one body into every widget
([docs/ux/widget.md](ux/widget.md) §8). Room, Hilt, kotlinx-serialization and
...
was twelve emoji drawn as text and the colour picker eight hues. The page
...
Two of the seven were not what this list thought, and reading the canvas rather
than drawing alternatives to it is what found both. The stars have no defect:
the app transcribes the canvas exactly, geometry, scale, rotation, opacity,
period and the second star's lag, and a measurement off the device agrees with
the specification to half a percent — so their drift is now a new decision
rather than a repair, and it moves the canvas boards as much as the code. The
eyes needed no options at all, because the canvas already carried exact
geometry for content's open eye and both the code and momo.md §3's own table
had drifted from it. Of the rest, the weeds turned out not to be fixable by
colour — the dark tank ended on a theme-invariant role a few units from the
weed's own, measuring 1.00:1, and neither more opacity nor a darker ink clears
both ends of the gradient — so the gradient changed instead. The habit question
went further than the four options: a habit keeps neither an icon nor a colour,
which deletes the pickers rather than replacing them. The pass also raised one
thing this list did not ask for, the Today row's inability to say what is owed
today, which is in 1.x rather than here. **The launcher closed last and took
the longest**, because removing the face removed the contrast that let a pale
mark sit on a pale ground: the mark is one gill cluster in Momo's own pink, and
it is the *ground* that moved, to `onPrimaryContainer`. Purple was drawn and
dropped, and so was a stalk connecting the cluster to a body
([docs/ux/visual-identity.md](ux/visual-identity.md) §7.1). **Step 2 is closed
entire.**

**Step 3 — Phase 1's own list, and what it left open.** Quick-complete actions as OQ-2 now reads,
with the notification's own details in [docs/ux/reminder.md](ux/reminder.md)
§4; gills as OQ-3 now reads, the canvas page done and marked, then the domain
value with JVM tests, then the panel; and Momo's real copy, drafted on the
canvas against alternatives and **settled as the lines already written**
...
belongs to; the panel and the drawing came with step 4, which rewrote the same
...
**Step 4 — polish, and it is closed.** Everything step 2 marked, item by item,
verified on the emulator against its artboard; and the naming sweep — Kotlin identifiers,
...
Three of the marked items turned out to be more than the list thought, and all
three the same way — the write-up had drifted from the canvas. The gills'
selection rule was missing the half that makes the count reachable at all; the
launcher mark's geometry lived only on the board, under a scale the document
quotes the *consequences* of without naming; and the habit icon and colour
reached 24 source files where this list had said eight. **What the canvas holds
and the document paraphrases is the thing to read**, which is the same lesson
step 2 recorded from the other side.

**Two things step 4 leaves owed**, both on running.md §4's boxes rather than in
the code: the launcher's three checks are unticked again, since the mark they
were ticked against is gone; and the reminder's small icon has still not been
...
with a baby Momo; more widgets (a stats one first — the API 31 size attributes
and picker previews [docs/ux/widget.md](ux/widget.md) §8 lists are the price of
a fourth provider; the Today, Streaks and Momo widgets already ship); themes,
Momo costumes and things in the tank
...
- **Streak resets causing abandonment.** Mitigation: OQ-3's escape hatch — gills, whose count is built (§5, §8); a run can now survive up to three finished misses rather than none, one spare earned per seven clean units, and step 4 drew the count, so a user can see what they have left — in the evening, on the habit closest to breaking, which is when and where it is worth knowing. **Half of this mitigation is gone**: "observe own behavior in a trial" had no replacement and was not given one, so what remains is the escape hatch without the evidence that would have sized it.
