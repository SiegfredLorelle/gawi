...
class WeekStreakTest {
    ...
    fun `two full weeks of three anywhere make a streak of two`() {
        ...
        assertEquals(2, Streaks.weekStreak(completions, timesPerWeek = 3, today = today))
    }
    ...
    fun `day placement within the week is irrelevant`() {
        ...
        assertEquals(1, Streaks.weekStreak(frontLoaded, timesPerWeek = 3, today = today))
        assertEquals(1, Streaks.weekStreak(backLoaded, timesPerWeek = 3, today = today))
    }
    ...
    fun `idempotent completions mean one distinct date cannot satisfy three`() {
        ...
        assertEquals(0, Streaks.weekStreak(singleDay, timesPerWeek = 3, today = today))
    }
    ...
    fun `a finished week below target resets the streak`() {
        ...
        assertEquals(0, Streaks.weekStreak(twoOfThreeLastWeek, timesPerWeek = 3, today = today))
    }
    ...
    fun `an in-progress week below target does not break the streak`() {
        ...
        assertEquals(1, Streaks.weekStreak(completions, timesPerWeek = 3, today = today))
    }
    ...
    fun `a current week that already hit extends the streak`() {
        ...
        assertEquals(2, Streaks.weekStreak(completions, timesPerWeek = 1, today = today))
    }
    ...
    fun `over-completing a week counts once`() {
        ...
        assertEquals(1, Streaks.weekStreak(fourDays, timesPerWeek = 3, today = today))
    }
    ...
    fun `the week start setting changes the bucketing`() {
        ...
        val mondayStart = Streaks.weekStreak(sundayAndMonday, 2, today, DayOfWeek.MONDAY)
        val sundayStart = Streaks.weekStreak(sundayAndMonday, 2, today, DayOfWeek.SUNDAY)
    ...
    fun `week streaks cross the year boundary without week-number bugs`() {
        ...
        val streak = Streaks.weekStreak(completions, 2, LocalDate.parse("2027-01-08"))
    ...
    fun `sunday start also crosses the year boundary cleanly`() {
        ...
        val streak = Streaks.weekStreak(completions, 2, LocalDate.parse("2027-01-08"), DayOfWeek.SUNDAY)
    ...
    fun `a zero-completion week between full weeks leaves only the latest`() {
        ...
        assertEquals(1, Streaks.weekStreak(completions, 2, LocalDate.parse("2026-08-18")))
    }
}
