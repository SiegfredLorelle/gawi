    ...
    companion object {
        ...
        fun at(progress: Float): CelebrationFrame {
            ...
            return CelebrationFrame(
                ...
                bubbles = BurstLanes.mapIndexed { lane, l ->
                    val q = (p - l.delay) / (1f - l.delay)
                    BurstBubble(lane, q.coerceIn(0f, 1f), if (q <= 0f) 0f else BURST_ALPHA * piecewise(q, BURST_STOPS))
                }.filter { it.alpha > 0f },
            )
        }

        /** Linear between keyframe stops, each `position to value`, as the CSS keyframes are. */
        private fun piecewise(p: Float, stops: List<Pair<Float, Float>>): Float {
 ...
 * of the animations gate, and the celebration that has to outlive the list
 * item the tank sits in.
 */
internal class TodayMotion(val animationsOn: Boolean, val celebration: CelebrationState)

/** The screen's [TodayMotion] for [mood]; remember it above any list that scrolls the tank. */
@Composable
internal fun rememberTodayMotion(mood: Mood): TodayMotion {
    val animationsOn by rememberAnimationsEnabled()
    return TodayMotion(animationsOn, rememberCelebration(mood, animationsOn))
}
...
internal fun DrawScope.drawCelebrationBurst(frame: CelebrationFrame, colour: Color) {
    ...
    frame.bubbles.forEach { b ->
        val lane = BurstLanes[b.lane]
        val x = size.width * lane.across
        val y = size.height - (BURST_START + b.progress * BURST_RISE) * dp
        val radius = lane.size / 2f * (0.4f + 0.7f * b.progress) * dp
        drawCircle(colour, radius, Offset(x, y), alpha = b.alpha)
    }
}

private class BurstLane(val across: Float, val size: Float, val delay: Float)
...
/** Bubbles start this far above the floor — under Momo's tail — and rise this far, dp. */
